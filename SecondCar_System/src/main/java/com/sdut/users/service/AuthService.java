...
import com.sdut.users.mapper.AuthDao;
import com.sdut.users.pojo.Auth;
import com.sdut.users.pojo.Message;
import com.sdut.users.util.HttpUtil;
import com.sdut.users.util.IPUtil;
    ...
    public Message addAuthInfo(HttpServletRequest request) {
        ...
        if (jsonObject != null) {
            ...
            if (status == 0) {
                ...
                address = "山东省淄博市"; // 解析失败，默认地址
            }
        } else {
            address = "山东省淄博市"; // 获取baidu接口数据失败
        }
        // 将token相关信息存入数据库中
